...
- 🧪 **导出条件表达式**：工具箱「条件表达式管理器导出」按自定义 where 条件导出 CONDITION，结果可批量导入
...
- ✏️ **创建脚本**：右键菜单弹出对话框创建新脚本，支持多种脚本类型、启动点配置、固定脚本名称、DataBean/角色/条件脚本自动命名和命名规范提示
- 📥 **导入脚本**：右键 JSON 文件导入脚本到 Maximo，支持创建和更新脚本
- 🚀 **多线程导出**：导出脚本/应用XML/MAXOBJECT 支持多线程并发，大幅提升导出速度
- ⚙️ **可调线程数**：每个导出功能可独立配置线程数（1~20），持久化保存
- 📦 **自动打包ZIP**：导出完成后自动打包为ZIP并删除源目录，节省磁盘空间
- 📋 **计划导出**：配置多组导出任务（MAXOBJECT/消息/域/条件表达式/脚本/应用XML），按顺序自动执行，支持变量替换
- 🔧 **模块配置**：计划导出任务列表中按导出类型动态显示模块专属配置项（where 条件、单文件行数、精简模式）
...
- 点击 **“条件表达式管理器导出”** 标签页
- 选择导出目录（自动保存）
- 输入 **where 过滤条件**（SQL 表达式，多行文本框，默认 `1=1` 导出全部，输入内容自动持久化保存）
- 点击 **“开始导出”** 按钮
- 调用 `POST /script/SKS_CONDITION_MANAGE?_action=export&_langcode=<语言>&ignoreDefVal=false`，请求体 `{"where": "<过滤条件>"}`
- 导出结果为 `conditions.json`（包含 `conditions` 数组），可直接 POST 到 `SKS_CONDITION_MANAGE?_action=deploy` 批量导入
...
**新增功能**：
- ✨ 创建脚本新增「角色脚本」类型：填写角色名称 (MAXROLE)，脚本名自动生成为 `MAXROLE.<角色名称>`
- ✨ 创建脚本「条件脚本」新增 CONDITIONNUM 输入：脚本名自动生成为 `COND.<CONDITIONNUM>`，自动生成 CUSTOMCONDITION 启动点与表达式内容注释
- 🧪 工具箱新增「条件表达式管理器导出」标签页：自定义 where 过滤条件（多行文本框、持久化保存），导出 conditions.json 可批量导入
- 📅 计划导出新增「导出条件表达式」任务选项，可配置 where 过滤条件

### v1.4.34 (2026-07-30)

**新增功能**：
- ⚡ 侧边栏新增「快捷代码」标签页，基于 YAML 配置文件快速插入代码片段
  - 支持 JavaScript / 应用XML / MAXOBJECT配置 三种类型同时显示
  - 支持搜索过滤、可折叠树形结构、变量替换对话框
...
**新增功能**：
- 🚀 工具箱导出功能增强：导出脚本、导出应用XML、导出MAXOBJECT 改为多线程并发导出
  - 每个标签页可独立配置线程数（范围 1~20，默认 5），配置持久化保存
  - 大幅提升大量脚本/应用/对象的导出速度
- 📦 新增导出完成后自动打包为ZIP功能
  - 每个标签页独立开关，配置持久化保存
  - 仅当自动生成时间戳子目录时生效
...
- 🐛 修复 completionMode 被 envs.json 意外覆盖的问题
  - completionMode 不再从 envs.json 中读取或保存
  - 完全由 VSCode 全局设置控制，避免切换环境后补全模式被篡改

### v1.4.21 (2026-07-12)

**新增功能**：
- ✨ 创建脚本对话框新增「固定脚本名称脚本」标签页
  - 支持 OBJECT.NEW_FIXED（新增触发脚本，系统固定名称 `<对象名>.NEW`）
  - 支持 OBJECT.SAVE_FIXED（保存触发脚本，系统固定名称 `<对象名>.SAVE`）
  - 输入对象名称或应用名称后自动生成脚本名
- ✨ 创建脚本对话框新增命名规范提示
  - 根据脚本类型（APISCRIPT、OBJECT.SAVE、FLD_ACTION 等）显示对应的命名建议
  - 帮助用户选择合适的脚本名称，参考 TASK09.md 整理的规范
- ✨ Pull 应用 XML 后自动刷新已打开的编辑器
...
**改进优化**：
- 🔧 SKS_EXPORT_DBCONFIG 脚本新增导出字段（triggerRoot、storagePartition 等）
- 🔧 SHARPTREE.AUTOSCRIPT.LIBRARY 脚本更新

### v1.4.19 (2026-07-12)

**新增功能**：
- ✨ HTTP 请求错误时自动保存详细错误信息到临时文件
...
- ✨ 新增 SKS_LOGGER_MANAGE 日志管理脚本

**修复**：
- 🐛 修复脚本配置中 `active` 字段值类型（`'Y'`/`'N'` 改为 `true`/`false`）
