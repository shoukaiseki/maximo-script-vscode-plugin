        ...
        for (var i = 0; i < userKeys.length; i++) {
            ...
            if (action === "disconnect") {
                ...
                if (targetUserId && uid.equals(targetUserId)) {
                    ...
                    // 跳过 sessionId=0 的无效记录，取真实 session ID
                    var maxsessionid = null;
                    for (var si = 0; si < sessionKeys.length; si++) {
                        var sid = sessionKeys[si];
                        // if (sid.longValue() > 0) {
                            logger.info("sid=" + sid + ", sid.longValue()=" + sid.longValue());
                            maxsessionid = sid;
                            securityService.disconnectUser(uid, maxsessionid.longValue(), 3, "SYSTEM");
                            loggerMX.error("已断开用户: " + uid + ", sessionId: " + maxsessionid);
                            // break;
                        // }
                    }
                    if (maxsessionid !== null) {
                        logger.info("MaxSessionID=" + maxsessionid);
                        // disconnectUser(String userid, long maxsessionid, int disconnectType, String adminUserID)
                        securityService.disconnectUser(uid, maxsessionid.longValue(), 3, "SYSTEM");
                        loggerMX.error("已断开用户: " + uid + ", sessionId: " + maxsessionid);
