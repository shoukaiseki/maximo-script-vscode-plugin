 ...
 * 脚本(AUTOSCRIPT): SHARPTREE.AUTOSCRIPT.INSTALL
 * 脚本语言(SCRIPTLANGUAGE): nashorn
 * 描述(DESCRIPTION): Sharptree Automation Script Install Script
 * 日志级别(LOGLEVEL): ERROR
 * 唯一标识(AUTOSCRIPTID): 259            语言代码(LANGCODE): ZH
 * 用户定义(USERDEFINED): Y               状态(STATUS): Draft
 * 是接口(INTERFACE): N                  活动(ACTIVE): Y
 * 变更人(CHANGEBY): MAXADMIN
 * 日期(CHANGEDATE): 2026/5/20 18:19:21
 *
 * Variables: 无
 *
 * Launch Points: 无
...
function addOrUpdateLogger(logger, level, parent) {
    ...
    try {
        ...
        service.log("-------MAXLOGGER.where="+sqlFormat.format())
        loggerSet.setWhere(sqlFormat.format());
        var child;
        ...
        if (loggerSet.isEmpty()) {
            ...
            child.setValue("PARENTLOGGERID",parent.getInt("MAXLOGGERID"))
            child.setValue("LOGGER", logger,11);
          child.setValue("LOGKEY",parent.getString("LOGKEY")+"."+logger);
