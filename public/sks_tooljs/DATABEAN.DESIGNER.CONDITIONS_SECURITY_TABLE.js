...
var scriptName = "DATABEAN.DESIGNER.CONDITIONS_SECURITY_TABLE"//service.getScriptName()
 ...
 * @param {psdi.webclient.system.beans.DataBeanContext} dbctx - 数据Bean上下文
 */
function fastAddConditionDisplayProp(dbctx) {
  initLogger(dbctx);
  ...
  if (!ctrlcondSetHasProp(dbctx, mboSetTrue, "display")) {
    var mboTrue = mboSetTrue.add()
    mboTrue.setValue("property", "display")
    mboTrue.setValue("propertyvalue", "true")
  }
  var mboSetFalse = condMbo.getMboSet("CTRLCONDPROPFALSE")
  if (!ctrlcondSetHasProp(dbctx, mboSetFalse, "display")) {
    var mboFalse = mboSetFalse.add()
    mboFalse.setValue("property", "display")
    mboFalse.setValue("propertyvalue", "false")
  }
  refreshDataBean(dbctx, "property_values_true_Table")
  refreshDataBean(dbctx, "property_values_false_Table")
...
function ctrlcondSetHasProp(dbctx, ctrlCondSet, propName) {
  initLogger(dbctx);
  for (var mbo = ctrlCondSet.moveFirst(); mbo != null; mbo = ctrlCondSet.moveNext()) {
    if (propName.equalsIgnoreCase(mbo.getString("property"))) {
      return true;
    }
  }
  return false;
...
/**
 ctrlCondSetBean
 * @param {psdi.webclient.system.beans.DataBeanContext} dbctx - 数据Bean上下文
 * @param {string} beanid - 数据Bean ID
 */
function refreshDataBean(dbctx, beanid) {
  initLogger(dbctx);
  var appInstance = dbctx.getAppInstance();
  var appBean = appInstance.getAppBean();
  /** @type {psdi.webclient.system.beans.DataBean} */
  var dataBean = appInstance.getDataBean(beanid);
